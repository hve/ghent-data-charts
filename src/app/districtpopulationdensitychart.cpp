...
DistrictPopulationDensityChart::DistrictPopulationDensityChart(DbService& dbservice, QWidget *parent)
...
bool DistrictPopulationDensityChart::doLoad()
{
    for (auto& district : mDbService.districts()) {

        auto result = mDbService.getPopulationDensitySeriesForDistrict(district.id);
        if (result.empty()) {
    ...
    axisY->setRange(0, 11000);  // TODO: make dynamic
    axisY->setTickType(QValueAxis::TicksFixed);
    axisY->setTickCount(12);
    axisY->setTitleText("Dichtheid (inwoners per km2)");
    ...
    for (QLegendMarker* marker : markers) {
        QObject::connect(marker, &QLegendMarker::clicked, this, &DistrictPopulationDensityChart::onMarkerClicked);
        if (marker_counter++ < 8)
...
void DistrictPopulationDensityChart::onMarkerClicked()
{
